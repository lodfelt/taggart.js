(function($){
	...
	var methods = {
		...
		tagable: function(method) {
			$(this).on("keydown", function(event) {
				var valueText, tags, $targetUL, textFromExistingTag;
				...
				if(event.keyCode === settings.submitButton) {
					valueText = $('ul.typeahead.dropdown-menu').is(':visible') ? $('ul.typeahead.dropdown-menu li.active').text() : $(this).val();
					if(assertTextGTE(valueText)) {
						$targetUL = $(this).parent('.input_container').siblings('.tag-container').children('ul');
						if($targetUL.children().length > 0) {
							tags = new Array();
							$targetUL.children().each(function() {
								textFromExistingTag = $(this).text().substring(0, $(this).text().lastIndexOf('X')).trim();
								tags.push(textFromExistingTag);
							});
						}
						if(tags.indexOf(valueText) == -1) {
	...
	function assertTextGTE(string, length) {
		if((string !== '') && (string.length >= settings.threshold))
			return true;
		return false;
	}
